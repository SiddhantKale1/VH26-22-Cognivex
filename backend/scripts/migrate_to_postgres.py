"""
Migration Script: Sync all existing raw PDF manuals into PostgreSQL raw_manuals table.
"""
...
def migrate():
    print("=" * 60)
    print("   PostgreSQL Manuals Migration Script")
    print("=" * 60)

    # Initialize database
    print(f"Connecting to PostgreSQL host={postgres_store.host}:{postgres_store.port} db={postgres_store.dbname}...")
    init_res = postgres_store.initialize_database()
    print(f"Initialization result: {init_res}")

    if init_res.get("status") != "connected":
        print(f"\n[ERROR] Could not connect to PostgreSQL: {init_res.get('error')}")
        print("Please verify your PostgreSQL credentials in backend/.env")
        return
    ...
    print(f"\nFound {len(pdf_files)} PDF manual(s) in {raw_dir}")
    ...
    for pdf_path in pdf_files:
        try:
            ...
            record = postgres_store.save_manual(
                filename=filename,
                content=content,
    ...
    print("-" * 60)
    print(f"Migration completed: {success_count}/{len(pdf_files)} manuals stored in PostgreSQL.")
