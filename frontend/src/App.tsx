...
export function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-[#0F172A] flex flex-col font-sans selection:bg-[#FFBB00] selection:text-[#0F172A]">
      ...
      {/* Main Body Layout with Permanent Sidebar */}
        ...
        <aside className="w-full md:w-64 shrink-0">
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm sticky top-22 space-y-4">
            <div>
              ...
              <nav className="space-y-1.5 flex md:flex-col gap-1.5 md:gap-0 overflow-x-auto pb-1 md:pb-0">
                {navItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-3 rounded-xl text-left transition-all cursor-pointer shrink-0 ${
                        isActive
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <span className={`text-[10px] hidden md:block ${isActive ? "text-slate-800" : "text-slate-400"}`}>
                            {item.subtitle}
                          </span>
                        </div>
                      </div>
                      <ChevronRight className={`w-3.5 h-3.5 opacity-60 hidden md:block ${isActive ? "text-slate-950" : "text-slate-400"}`} />
                    </button>
                  );
                })}
              </nav>
            ...
            {/* Architecture Quick Highlights Box */}
