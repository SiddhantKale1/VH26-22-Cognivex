...
const LANGUAGE_OPTIONS = [
  ...
  { id: "auto", label: "Auto-Detect Query Language" },
];
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-2">
          ...
          {/* Target Output Language Badge */}
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-700">
            <Globe2 className="w-3.5 h-3.5 text-amber-500" />
            <span>Output: {LANGUAGE_OPTIONS.find(l => l.id === selectedLanguage)?.label.split(" ")[0]}</span>
          </div>
        </div>

        <p className="text-sm text-slate-500 mb-6">
          Type or use the <strong>Voice button</strong> to ask a question in English, हिन्दी, 日本語, Deutsch, etc. (e.g. <code className="text-amber-800 font-mono bg-amber-50 border border-amber-200 px-2 py-0.5 rounded font-bold">Fault F07900</code>, <code className="text-amber-800 font-mono bg-amber-50 border border-amber-200 px-2 py-0.5 rounded font-bold">8013</code>).
        </p>
        ...
        >
          ...
          <div className="flex flex-col md:flex-row gap-2.5 items-stretch">
            {/* VOICE BUTTON AT THE LEFT SIDE OF TEXT INPUT QUERY */}
            <button
              type="button"
              onClick={toggleVoiceInput}
              className={`px-4 py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer font-bold text-xs shadow-xs border shrink-0 ${
                isListening
                  ? "bg-red-600 hover:bg-red-700 text-white border-red-700 shadow-md shadow-red-200 ring-2 ring-red-400 animate-pulse"
                  : "bg-slate-100 hover:bg-amber-100 hover:border-amber-300 text-slate-700 hover:text-amber-950 border-slate-300"
              }`}
              title={isListening ? "Click to Stop Voice Recording" : `Click to Speak Voice Query (${LANGUAGE_OPTIONS.find(l => l.id === selectedLanguage)?.label.split(" ")[0]})`}
            >
              {isListening ? (
                <>
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className={`w-full pl-12 pr-4 py-3.5 bg-slate-50 border rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-amber-500 focus:bg-white focus:ring-2 focus:ring-amber-200 transition-all text-sm shadow-inner ${
            ...
            <select
              ...
              className="px-3.5 py-3.5 bg-slate-50 border border-slate-300 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-amber-500 focus:bg-white cursor-pointer font-medium shrink-0"
            >
            ...
            <div className="relative shrink-0">
              <select
                ...
                className="px-3.5 py-3.5 bg-slate-50 border border-slate-300 rounded-xl text-slate-800 text-sm focus:outline-none focus:border-amber-500 focus:bg-white cursor-pointer font-medium"
                title="Select Output Diagnosis Language"
              >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-6 py-3.5 bg-[#FFBB00] hover:bg-amber-400 text-slate-950 font-bold rounded-xl text-sm transition-all shadow-md shadow-amber-200 border border-amber-300 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shrink-0"
            >
              {loading ? (
                <>
